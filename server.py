...
def has_pending_action(session: dict[str, Any]) -> bool:
    return bool(session.get("pending_action") or session.get("pending_days") or session.get("pending_start"))


def classify_intent(message: str, history: list[dict[str, str]], session: dict[str, Any] | None = None) -> dict[str, Any]:
    text = message.lower()
    policy_words = {"policy", "maternity", "paternity", "payroll", "payslip", "onboarding", "probation", "sick"}
    action_words = {"apply", "request", "book", "balance", "leave", "fetch", "download", "get", "start", "date"}
    pending_action = has_pending_action(session or {})
    wants_policy = any(word in text for word in policy_words)
    wants_action = pending_action or (any(word in text for word in action_words) and ("policy" not in text or "apply" in text))
...
def parse_leave_request(message: str, session: dict[str, Any]) -> dict[str, Any]:
    ...
    date_pattern = r"(?:\d{4}-\d{1,2}-\d{1,2}|\d{1,2}[/-]\d{1,2}(?:[/-]\d{2,4})?|(?:\d{1,2}(?:st|nd|rd|th)?\s*)?(?:jan|january|feb|february|mar|march|apr|april|may|jun|june|jul|july|aug|august|sep|sept|september|oct|october|nov|november|dec|december)\s*\d{0,2}(?:st|nd|rd|th)?|(?:jan|january|feb|february|mar|march|apr|april|may|jun|june|jul|july|aug|august|sep|sept|september|oct|october|nov|november|dec|december)\s*\d{1,2}(?:st|nd|rd|th)?)"
    start_match = re.search(rf"(?:starting(?:\s+from)?|start(?:\s+date\s+is)?|from|on)\s+({date_pattern})", text)
    if not start_match and has_pending_action(session):
        start_match = re.search(date_pattern, text)
    leave_type = "sick" if "sick" in text else session.get("pending_leave_type", "annual")
...
def parse_date(raw: str | None) -> date | None:
    ...
    raw = re.sub(r"\s+", " ", raw)
    raw = re.sub(r"^([a-z]+)(\d{1,2})$", r"\1 \2", raw)
    raw = re.sub(r"^(\d{1,2})([a-z]+)$", r"\1 \2", raw)
    month_aliases = {
        "jan": "january",
        "feb": "february",
        "mar": "march",
    ...
    parts = raw.split()
    if parts:
        parts = [month_aliases.get(part, part) for part in parts]
        raw = " ".join(parts)
    for fmt in ("%Y-%m-%d", "%d/%m/%Y", "%d-%m-%Y", "%d/%m/%y", "%d-%m-%y", "%d/%m", "%d-%m", "%B %d", "%d %B"):
        try:
...
def action_agent(message: str, session: dict[str, Any], trace: Trace) -> dict[str, Any]:
    ...
    if not parsed.get("days") or not parsed.get("start"):
        session["pending_action"] = "leave_apply"
        session["pending_days"] = parsed.get("days")
        session["pending_start"] = parsed.get("start")
        session["pending_leave_type"] = parsed.get("leave_type")
    ...
    if result.get("ok"):
        for key in ("pending_action", "pending_days", "pending_start", "pending_leave_type"):
...
def handle_chat(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    trace = Trace()
    route = trace.run("orchestrator-agent", "classify_and_route", {"message": message, "history_tail": session["history"][-4:], "pending_action": session.get("pending_action")}, lambda: classify_intent(message, session["history"], session))
